...
public class LoginActivity extends Activity {
	...
	protected void onNewIntent(Intent intent) {
		...
		if (intent.getData() != null && intent.getData().getQueryParameter("access_token") != null) {
			...
			String refreshToken = data.getQueryParameter("refresh_tokem");
			String expiresInString = data.getQueryParameter("expires_in");
			
			PreferencesWrapper prefs = new PreferencesWrapper(this);
			prefs.setAccessToken(accessToken);
			prefs.setRefreshToken(refreshToken);
			prefs.setTokenExpirationTime(System.currentTimeMillis() + (Long.parseLong(expiresInString) * 1000));
			
			UserInfoTask userInfoTask = new UserInfoTask(this, prefs, mRegistryClient);
			userInfoTask.execute(accessToken);
		}
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.social_account_login_button).setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				//Intent socialAccountLoginIntent = new Intent(LoginActivity.this, SocialAccountLoginActivity.class);
				//startActivity(socialAccountLoginIntent);
				Intent socialLogin = new Intent(Intent.ACTION_VIEW, Uri.parse("http://grant.crowdsos.net/accounts/facebook/login/?method=js_sdk"));
	...
	public void attemptLogin() {
		...
		if (cancel) {
			...
			showProgress(true);
			mAuthTask = new UserLoginTask(this, new PreferencesWrapper(this), mRegistryClient) {
				@Override
